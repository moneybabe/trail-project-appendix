...
def s_i(x: ctx_iv.ivmpf, i: int, stored_val: dict = {}) -> ctx_iv.ivmpf:
    if f"s_{i}" in stored_val:
        return stored_val[f"s_{i}"]

    output = x  # s_0 = x
    if i > 0:
        output = s(s_i(x, i - 1, stored_val))  # s_i = s(s_{i-1}(x)); i > 0
    elif i < 0:
        output = s_inverse(s_i(x, i + 1, stored_val))  # s_i = s_{-1}(s_{i+1}(x)); i < 0
...
def P_i(x: ctx_iv.ivmpf, i: int, print_val: bool = False) -> ctx_iv.ivmpf:  # i >= 0
    ...
    for j in range(i):  # j = [0, 1, ..., i - 1]
        ...
        output = iv.fadd(
            output, product
        )  # P_i = 1 + (c_0 - 1) + (c_0 - 1)(c_1 - 1) + ... + (c_0 - 1)(c_1 - 1)...(c_{i-1} - 1)

    if print_val:
        headers = ["Function", "Value"]
        data = [[key, stored_val[key]] for key in sorted(stored_val.keys())]
        data.append(["P_" + str(i), output])
...
def S_i(x: ctx_iv.ivmpf, i: int, print_val: bool = False) -> ctx_iv.ivmpf:  # i >= 0
    ...
    for j in range(i):  # j = [0, 1, ..., i - 1]
        ...
        output = iv.fadd(
            output, product
        )  # S_i = 1 + (d_0 - 1) + (d_0 - 1)(d_1 - 1) + ... + (d_0 - 1)(d_1 - 1)...(d_{i-1} - 1)

    if print_val:
        headers = ["Function", "Value"]
        data = [[key, stored_val[key]] for key in sorted(stored_val.keys())]
        data.append(["S_" + str(i), output])
        print(tabulate(data, headers=headers, tablefmt="grid"))
...
def Q_i(x: ctx_iv.ivmpf, i: int, print_val: bool = False) -> ctx_iv.ivmpf:  # i >= 1
    ...
    for j in range(1, i):  # j = [1, 2, ..., i - 1]
        ...
        output = iv.fadd(
            output, product
        )  # Q_i = 0 + (c_{-1} - 1) + (c_{-1} - 1)(c_{-2} - 1) + ... + (c_{-1} - 1)(c_{-2} - 1)...(c_{-i+1} - 1)

    if print_val:
        headers = ["Function", "Value"]
        data = [[key, stored_val[key]] for key in sorted(stored_val.keys())]
        data.append(["Q_" + str(i), output])
        print(tabulate(data, headers=headers, tablefmt="grid"))
...
def T_i(x: ctx_iv.ivmpf, i: int, print_val: bool = False) -> ctx_iv.ivmpf:  # i >= 1
    ...
    for j in range(1, i):  # j = [1, 2, ..., i - 1]
        ...
        for k in range(1, j + 1):  # k = [1, 2, ..., j]
            ...
            product = iv.fmul(
                product, new_product
            )  # (d_{-1} - 1)(d_{-2} - 1)...(d_{-k} - 1)

        output = iv.fadd(
            output, product
        )  # T_i = 0 + (d_{-1} - 1) + (d_{-1} - 1)(d_{-2} - 1) + ... + (d_{-1} - 1)(d_{-2} - 1)...(d_{-i+1} - 1)

    if print_val:
        headers = ["Function", "Value"]
        data = [[key, stored_val[key]] for key in sorted(stored_val.keys())]
        data.append(["T_" + str(i), output])
        print(tabulate(data, headers=headers, tablefmt="grid"))
...
def M_lk(x: ctx_iv.ivmpf, l: int, k: int, print_val: bool = False) -> ctx_iv.ivmpf:
    ...
    if print_val:
        print(f"M_{l},{k} ({x}) = {output}\n")

    return output


def M_lk_down(
    a: ctx_iv.ivmpf, b: ctx_iv.ivmpf, l: int, k: int, print_val: bool = False
) -> ctx_iv.ivmpf:
    ...
    if print_val:
        print(f"M_{l},{k}^down ({a}, {b}) = {output}\n")

    return output


def M54_down(a: ctx_iv.ivmpf, b: ctx_iv.ivmpf, print_val: bool = False) -> ctx_iv.ivmpf:
    return M_lk_down(a, b, 5, 4, print_val)
